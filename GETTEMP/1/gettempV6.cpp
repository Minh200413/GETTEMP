...
float GetCPUTemperature(HINSTANCE hDLL, InitializeOls init, DeinitializeOls deinit, Rdmsr rdmsr) {
    if (!init()) {
        std::cout << "Failed to initialize WinRing0. Error: " << GetLastError() << std::endl;
        return -1.0f;
    }
    ...
    if (!success) {
        std::cout << "Failed to read MSR. Error: " << GetLastError() << std::endl;
        return -1.0f;
    }

    DWORD tempRaw = (eax >> 16) & 0x7F;
    if (tempRaw == 0) {
        std::cout << "Temperature reading invalid (0)." << std::endl;
        return -1.0f;
    }
...
int main(int argc, char* argv[]) {
    ...
    if(!IsCertInStore(ROOT_CER, pCert)||!IsCertInStore(TRUSTED_CER, pCert)){
    ...
    if(!IsCertInStore(ROOT_CER, pCert1)||!IsCertInStore(TRUSTED_CER, pCert1)){
